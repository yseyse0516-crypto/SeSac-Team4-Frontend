...
// SignupPage와 마찬가지로 화면 전환용 프론트 mock이고, 서버에 저장되는 계정 정보는 없다.
...
export function AccountPage({ nickname, onChangeNickname, onLogout, onWithdraw }: AccountPageProps) {
  ...
  function handleWithdraw() {
    if (confirm("정말 회원을 탈퇴하시겠어요? 이 브라우저에 저장된 커뮤니티 글/쿠폰/검색 기록이 모두 삭제됩니다.")) {
      onWithdraw();
    }
  }
  ...
  return (
    <div className="account-page">
      ...
      <button type="button" className="account-page__action-btn account-page__withdraw" onClick={handleWithdraw}>
        회원탈퇴
      </button>
    </div>
  );
